...
* Last Modified..: $Date: 2004/05/30 23:30:18 $
	...
	function dynCalendar_show()
	{
		...
		while(i <= numdays){
            ...
            */
			...
			linkHTML = '<a class="dynCalendar_daylink" href="javascript: ' + this.callbackFunc + '(' + i + ', ' + (Number(month) + 1) + ', ' + year + '); ' + this.objName + '._hideLayer()">' + (i++) + '</a>';
...
*/
	function dynCalendar_writeHTML()
	{
		if (is_ie5up || is_nav6up || is_gecko) {
			...
			//Added by mvsxyz
			document.write('<iframe id="iframe_' + this.layerID + '" style="display:none;position: absolute;top: 1px;left: 1px;z-index:999;height:140px;width:180px;visibility:hidden;border:0;">&nbsp;</iframe>');
			//Edit by mvsxyz
      document.write('<div class="dynCalendar" id="div_' + this.layerID + '" onmouseover="' + this.objName + '._mouseover(true)" onmouseout="' + this.objName + '._mouseover(false)" style="z-index:1000;"></div>');
		}
	}
...
* @access private
*/
	function dynCalendar_getLayer(typ)
	{
		var layerID = this.layerID;

		//Added by mvsxyz
    if(typ == 'iframe') {
		    if (document.getElementById('iframe_'+layerID)) {

			      return document.getElementById('iframe_'+layerID);

		    } else if (document.all('iframe_'+layerID)) {
			      return document.all('iframe_'+layerID);
		    }		

    //Edit by mvsxyz
		} else {
		    if (document.getElementById('div_'+layerID)) {

			      return document.getElementById('div_'+layerID);

		    } else if (document.all('div_'+layerID)) {
			      return document.all('div_'+layerID);
		    }
		}
	}
...
*/
	...
	{
		this._getLayer('div').style.visibility = 'hidden'; //Edit by mvsxyz
		this._getLayer('iframe').style.display = 'none'; //Added by mvsxyz
	}
...
*/
	...
	{
		this._getLayer('div').style.visibility = 'visible'; //Edit by mvsxyz
		this._getLayer('iframe').style.display = 'block';  //Added by mvsxyz
	}
...
*/
	...
	{
		this._getLayer('div').style.top  = (dynCalendar_mouseY + this.offsetY) + 'px'; //Edit by mvsxyz
		this._getLayer('div').style.left = (dynCalendar_mouseX + this.offsetX) + 'px'; //Edit by mvsxyz
		this._getLayer('iframe').style.top  = (dynCalendar_mouseY + this.offsetY) + 'px'; //Added by mvsxyz
		this._getLayer('iframe').style.left = (dynCalendar_mouseX + this.offsetX) + 'px'; //Added by mvsxyz
	}
...
*/
	...
	{
		this._getLayer('div').innerHTML = html; //Edit by mvsxyz
	}
