...
if(!is_object($SF_user))
	$sf_user =& $GLOBALS['sf_factory']->getObject('ADMINISTRATION', 'User');
else 
	$sf_user =& $SF_user;
...
$_AS['temp']['sortlinkelements']['EDR']='userid';
...
foreach ( $_AS['temp']['list_output_header_fieldarr'] as $kk => $vv) {
	...
	if ($vv=='editor') 
			$_AS['temp']['label']=$_AS['article_obj']->lang->get('editor');
...
if(strpos($_AS['action'],'export_facebook')!==false) 
	include_once $_AS['basedir'] . 'inc/inc.export.facebook_create.php';
...
foreach ($_AS['items_arrays'] as $v) {
    ...
    $_AS['temp']['list_output_fields_arr'] = $_AS['temp']['list_output_header_fieldarr'];
		...
		foreach ($_AS['temp']['list_output_fields_arr'] as $kk => $vv){
			...
			// list editor
			if ($vv=='editor'){
				$_AS['temp']['field_value']=$sf_user->getName($sf_user->loadByIduser($_AS['current_item']['userid'])).' '.$sf_user->getSurname($sf_user->loadByIduser($_AS['current_item']['userid']));
				if (trim($_AS['temp']['field_value'])=='')
					$_AS['temp']['field_value']=$sf_user->getUsername($sf_user->loadByIduser($_AS['current_item']['userid']));
			}
			...
			if (strpos($vv,'custom')!==false) {
				...
				if ($_AS['temp']['field_type']=='pic' ) {
						...
						if (trim($_AS['temp']['value_arr'][0])!='')
							$_AS['temp']['field_value']='<img src="'.$cfg_client["htmlpath"].substr(trim($_AS['temp']['value_arr'][0]),0, strrpos(trim($_AS['temp']['value_arr'][0]),".")).$cfg_client["thumbext"].strtolower(substr(trim($_AS['temp']['value_arr'][0]),strrpos(trim($_AS['temp']['value_arr'][0]),"."))).'" alt=""/>';
						else
							$_AS['temp']['field_value']='&nbsp;';
				}
				if ($_AS['temp']['field_type']=='link' ||
						$_AS['temp']['field_type']=='file') {
						$_AS['temp']['value_arr']=explode("\n",$_AS['temp']['field_value']);
						$_AS['temp']['field_value']=trim($_AS['temp']['value_arr'][0]);//url
						if ($_AS['temp']['field_value']=='')
							$_AS['temp']['field_value']='&nbsp;';
				}
    ...
    //Aktionslinks
	    ...
	    if($_AS['current_item']['online'] == 1) {
	   		if ($_AS['current_item']['article_enddate_yn']!=1 && $valid===true && trim($_AS['current_item'][$_AS['article_obj']->getSetting('spfnc_facebook_lastsent_postid_cf')])=='' ){
		    	$_AS['temp']['list_output'] = str_replace('{facebook}', '<a href="'.$sess->url($cfg_cms['cms_html_path'].'main.php?area=plugin&cms_plugin='.$_AS['db'].'/index.php&subarea='.$_AS['subarea'].'&action=export_facebook&idarticle='.$_AS['current_item']['idarticle'].'&page='.$_AS['current_page'].$_AS['temp']['lv_cf']['urladdon'].'&callist_flt_category='.$_AS['cms_wr']->getVal('callist_flt_category').'&callist_search='.$_AS['temp']['searchstring'].'&startmonth='.$_AS['config']['startmonth'].'&monthback='.$_AS['config']['monthback'].'&sort='.$_AS['temp']['sortlinkvalsoldstring']).'"><img src="plugins/articlesystem/tpl/'.$_AS['article_obj']->getSetting('skin').'/img/but_facebook.png" width="16" height="16" border="0" title="'.$_AS['article_obj']->lang->get('facebook').'"/></a>&nbsp;&nbsp;', $_AS['temp']['list_output']);
	   		} else if ($_AS['current_item']['article_enddate_yn']==1 || $valid!==true ){
	   		...
	   		} else if ($_AS['current_item']['article_enddate_yn']!=1 && $valid===true &&  trim($_AS['current_item'][$_AS['article_obj']->getSetting('spfnc_facebook_lastsent_postid_cf')])!='' ){
		    	$_AS['temp']['list_output'] = str_replace('{facebook}', '<a href="'.$sess->url($cfg_cms['cms_html_path'].'main.php?area=plugin&cms_plugin='.$_AS['db'].'/index.php&subarea='.$_AS['subarea'].'&action=export_facebook&idarticle='.$_AS['current_item']['idarticle'].'&page='.$_AS['current_page'].$_AS['temp']['lv_cf']['urladdon'].'&callist_flt_category='.$_AS['cms_wr']->getVal('callist_flt_category').'&callist_search='.$_AS['temp']['searchstring'].'&startmonth='.$_AS['config']['startmonth'].'&monthback='.$_AS['config']['monthback'].'&sort='.$_AS['temp']['sortlinkvalsoldstring']).'"><img src="plugins/articlesystem/tpl/'.$_AS['article_obj']->getSetting('skin').'/img/but_facebook_on.png" class="disabled" width="16" height="16" border="0" title="'.$_AS['article_obj']->lang->get('facebook').'"/></a>&nbsp;&nbsp;', $_AS['temp']['list_output']);
				} else
		    	$_AS['temp']['list_output'] = str_replace('{facebook}', '<img src="plugins/articlesystem/tpl/'.$_AS['article_obj']->getSetting('skin').'/img/but_facebook.png" width="16" height="16" border="0" title="'.$_AS['article_obj']->lang->get('facebook').'" class="disabled"/>&nbsp;&nbsp;', $_AS['temp']['list_output']);
...
$_AS['output']['list_range_prevnext'] = str_replace(array(
      ...
      array(
        ...
        '<form name="form_callist_flt_category" method="post" action="main.php">
            <input type="hidden" name="area" value="'.$_AS['cms_wr']->getVal('area').'">
            <div style="display:none;"><input type="hidden" name="cms_plugin" value="'.$_AS['cms_wr']->getVal('cms_plugin').'">
            <input type="hidden" name="subarea" value="'.$_AS['cms_wr']->getVal('subarea').'">
            <input type="hidden" name="action" value="'.$_AS['cms_wr']->getVal('action').'">
            ...
            '.$_AS['article_obj']->getSelectUni(	'callist_flt_category',
																							 ...
																							 $_AS['select']['catdata'],
																								'callist_flt_category',
																								'form_callist_flt_category.submit();return false;',
																								 (($_AS['article_obj']->getSetting('lv_show_catfilter')!='false' &&
																								 count($_AS['select']['catdata'])>1)? '' : ' style="display:none;"')).
					$_AS['temp']['lv_cf']['html'].'
        </form>',
...
#            '{select_turnus}',
      ...
      (($_AS['article_obj']->getSetting('spfnc_facebook')!='true')?'display:none;':'display:none;'),
