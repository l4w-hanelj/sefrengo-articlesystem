...
#		if (empty($_POST['article'][$v])){
		if (trim($_POST['article'][$v])==''){
			$_AS['valid']='false';
			$_AS['validation_errors'][$v]['text'][]= $_AS['article_obj']->lang->get('article_value_missing');
		}
...
if (trim($_POST['article']['title'])=='') {
	$_AS['valid']='false';
	$_AS['validation_errors']['title']['text'][]= $_AS['article_obj']->lang->get('article_value_missing');
}
...
foreach ($_AS['validation_fields'] as $v)
	if ($_AS['article_obj']->getSetting('article_'.$v.'_validation')=='true' &&
		...
		if ($v=='picture1' && count($_AS['validation_image'])<1) {
			...
			$_AS['validation_errors']['pictures']['text'][]= $_AS['article_obj']->lang->get('article_value_missing');
			}	
		if ($v=='file1' && count($_AS['validation_file'])<1) {
			$_AS['valid']='false';
			$_AS['valid2']['file']=false;
			$_AS['validation_errors']['files']['text'][]= $_AS['article_obj']->lang->get('article_value_missing');
			}	
		if ($v=='link' && count($_AS['validation_link'])<1) {
			$_AS['valid']='false';
			$_AS['valid2']['link']=false;
			$_AS['validation_errors']['links']['text'][]= $_AS['article_obj']->lang->get('article_value_missing');
			}	
		if ($v=='date' && count($_AS['validation_date'])<1) {
			$_AS['valid']='false';
			$_AS['valid2']['date']=false;
			$_AS['validation_errors']['dates']['text'][]= $_AS['article_obj']->lang->get('article_value_missing');
			}
		}


// check for installation of global ckEditor
if(file_exists($cfg_client['path'] . 'cms/ckeditor/')) {
	//Tpl einladen (ckeditor)
	include_once $_AS['basedir'] . 'tpl/'.$_AS['article_obj']->getSetting('skin').'/tpl.article_edit.php';
} else {
	//Tpl einladen (tiny_mce)
	include_once $_AS['basedir'] . 'tpl/'.$_AS['article_obj']->getSetting('skin').'/tpl.article_edit_tiny.php';
}
...
if( $_AS['action']=='edit_article' || 
    ...
    if (isset($change_show_tree)) {
		...
		// load article data via id if article is already exists
...
*/
		// load newer article post data
		if (	($_POST['action']=='save_article' && $_AS['valid']=='false')  ||
					$_AS['cms_wr']->getVal('apply')=='3' ) {
...
//Artikel speichern
    ...
    //Wenn die Sprache gewechselt wurde, dann hier zur �bersicht, da der Eintrag nur f�r alte Sprache gilt
    if (isset($change_show_tree)) {
        echo $_AS['temp']['url'] = $sess->urlRaw($cfg_cms['cms_html_path'].'main.php?area=plugin&lang='.$lang.'&cms_plugin='.$_AS['db'].'/index.php&subarea='.$_AS['subarea'].'&action=show_article&page='.$_AS['cms_wr']->getVal('page').$_AS['temp']['lv_cf']['urladdon'].'&callist_flt_category='.$_AS['cms_wr']->getVal('callist_flt_category'));
      	header ('Location:' . $_AS['temp']['url'] );
      	exit;
    ...
    //Bei Bearbeiten: Wenn ID vorhanden -> hinzuf�gen
...
#						$_AS['elements']->delete( $_AS['cms_wr']->getVal('artelsel'));
    ...
    //Speichern -> zur�ck zur �bersicht
      if($_AS['cms_wr']->getVal('apply')==0) {
          ...
          $_AS['action'] = 'edit_article'; //Aktion nach dem "�bernehmen" auf Bearbeiten setzen
          $_AS['idarticle'] = &$_AS['item']->getDataByKey('idarticle'); //ID nach dem �bernehmen noch holen, falls ein neuer Eintrag �bernommen wurde
					$_AS['item']->loadById($_AS['idarticle']);
					$_AS['elements']->loadById($_AS['idarticle'], true);
		    ...
		    array(
		        ...
		        $_AS['article_obj']->getSelectUni(	'article[idcategory][]',
																	 ...
																	 $_AS['select']['catdata'],
																		'category',
																		'',
																		(($_AS['article_obj']->getSetting('set_category_multiple')==1)?'multiple="multiple" size="'.(count($_AS['select']['catdata'])<8?count($_AS['select']['catdata']):'5').'"':''))
		    ),
		    $_AS['tpl']['category']);
}
	  ...
	  $rb_link_url = $rb->exportConfigURL();
		...
		$_AS['temp']['row']['custom'.$i] = str_replace(array(
		            '{custom_field_raw_value}',
			          ...
			          '{select_picture_upload_folder}'
							  ...
							  $_AS['temp']['values']['value_txt'][$i]:$_AS['new_uploaded_picture']),
								...
								$_AS['article_obj']->lang->get('article_picture_link'),
								$_AS['article_obj']->lang->get('sort_index_input_title'),
								$_AS['temp']['values']['description'][$i],
								$_AS['temp']['values']['title'][$i],	            
			...
			$_AS['row']['custom'.$i] = str_replace(array(
						...
						array(
							strpos($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),'[')!==false ? trim(substr($_AS['article_obj']->getSetting('article_custom'.$i.'_label'), 0, strpos($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),'[')-1)) : $_AS['article_obj']->getSetting('article_custom'.$i.'_label'),
							'style="'.str_replace( array('[readonly]','[CSS',']'), 
																		 array('','',''),
																		 trim(trim(substr($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),strpos($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),'[')-1)))).'"',
							$_AS['article_obj']->lang->get('article_elements_sel'),
							$_AS['article_obj']->lang->get('article_elements_del'),
	    ...
	    $_AS['row']['custom'.$i] = str_replace(array(
	        ...
	        array(
	            strpos($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),'[')!==false ? trim(substr($_AS['article_obj']->getSetting('article_custom'.$i.'_label'), 0, strpos($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),'[')-1)) : $_AS['article_obj']->getSetting('article_custom'.$i.'_label'),
							'style="'.str_replace( array('[readonly]','[CSS',']'), 
																		 array('','',''),
																		 trim(substr($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),strpos($_AS['article_obj']->getSetting('article_custom'.$i.'_label'),'[')-1))).'"',
	            $_AS['article_obj']->lang->get('article_elements_sel'),
	            $_AS['article_obj']->lang->get('article_elements_del'),
	            $_AS['article_obj']->lang->get('add'),
	          	((($_AS['article_obj']->getSetting('article_custom'.$i.'_validation')=='true' &&	
...
if ($_POST['action']=='save_article' && $_AS['valid']!='true')
  ...
  foreach ($_AS['validation_errors'] as $v => $k){
  	if ( count($_AS['validation_errors'][$v]['text'])>0 ) {
			if (is_array($_AS['validation_errors'][$v]['text']))
				foreach ($_AS['validation_errors'][$v]['text'] as $w)
				if (strpos($v,'custom')===false)
				 		$_AS['validation_error_msgs'][] = ($_AS['article_obj']->getSetting('article_'.$v.'_label')==''?$_AS['article_obj']->lang->get('article_'.$v):$_AS['article_obj']->getSetting('article_'.$v.'_label')).': '.$w;
					else
						$_AS['validation_error_msgs'][] = strpos($_AS['article_obj']->getSetting('article_'.$v.'_label'),'[')!==false ? trim(substr($_AS['article_obj']->getSetting('article_'.$v.'_label'), 0, strpos($_AS['article_obj']->getSetting('article_'.$v.'_label'),'[')-1)) : $_AS['article_obj']->getSetting('article_'.$v.'_label').': '.$w;
		}
} 

if (count($_AS['validation_error_msgs'])>0){
	echo '<table border="0" class="errormsg">';
 	foreach ($_AS['validation_error_msgs'] as $v)
 		echo '<tr><td valign="top" width="125" style="padding-left:30px;"><strong>'.str_replace(':','</strong></td><td valign="top">',$v).'</td></tr>';
	echo '</table>';
}
...
if (trim($_AS['hash'])=='')
	$_AS['hash']=$_AS['item']->getDataByKey('hash');
	
if (count($_AS['temp']['othlangcopyselarr'])>1 && $_AS['idarticle']!='' && trim($_AS['hash'])!='')
