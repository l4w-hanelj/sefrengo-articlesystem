...
if(! function_exists(as_str_chop)){
	...
	*/
	function as_str_chop($string, $length = 60, $center = true, $append = null, $txttransfrom=false)
	{
...
if(! function_exists(as_element_replacement)){
  ...
  */
	function as_element_replacement($item_obj,
																	...
																	$cfg_arr=array(),
																	$txttransform=true,
																	$active_customs=array()) {
		global $cfg_client,$client,$lang,$idcat,$idcatside,$lang,$as_externalvars2elements;		
		
		if (count($active_customs)==0)
			for ($i=1;$i<51;$i++){
				$active_customs[]=$i;
			}
	  ...
	  $repl_arr['title_urlencode']	=	urlencode($repl_arr['title']);
...
#	  $repl_arr['turnus_type']	=	$artsys_obj->lang->get('turnus_'.$item_obj->getDataByKey('turnus_type'));
		...
		//customs
//		for ($i=1;$i<51;$i++){
		foreach ($active_customs as $i){
	
			$repl_arr['custom:'.$i] = '';
			$repl_arr['custom_urlencode:'.$i] = '';
			$repl_arr['custom_label:'.$i] = '';
			$repl_arr['custom_data:'.$i] = '';
			$repl_arr['custom_data_urlencode:'.$i] = '';
			...
			$repl_arr['custom_linkidcattree:'.$i] = '';
			$repl_arr['custom_linkidcat:'.$i.':side_links'] = '';
			$repl_arr['custom_linkidcat:'.$i.':cat_links'] = '';
			...
			$repl_arr['custom_linkidcatsidetree:'.$i] = '';
		...
		// old
	      ...
	      $repl_arr['custom:'.$i]								=	$ctemp['values']['link'];
		      ...
		      $repl_arr['custom_linkidcat:'.$i]	=	(int) str_replace('cms://idcat=','',$repl_arr['custom:'.$i]);
				...
				if (strpos($tpl_main,'_linkidcattree')!==false && !empty($repl_arr['custom_linkidcat:'.$i])) {
					$treearray=array_merge(as_get_idcat_parent_cats($repl_arr['custom_linkidcat:'.$i],$lang,$client),
																 as_get_idcat_children_cats($repl_arr['custom_linkidcat:'.$i],$lang,$client));
					$repl_arr['custom_linkidcattree:'.$i] = implode(',',$treearray);
				} else
					$repl_arr['custom_linkidcattree:'.$i] = '';	


				if (!empty($repl_arr['custom_linkidcat:'.$i]) && strpos($tpl_main,':idcatside_si')!==false) {
				...
				if (!empty($repl_arr['custom_linkidcat:'.$i]) && strpos($tpl_main,':idcat_si')!==false) {
				...
				if (!empty($repl_arr['custom_linkidcat:'.$i]) && strpos($tpl_main,':cat_links')!==false) {

					$treebranch_array=array();
					$treebranch_data_array=array();
					$treebranch_array=as_get_idcat_children_cats($repl_arr['custom_linkidcat:'.$i],$lang,$client);
					$treebranch_data_array=as_build_treebranch_data_array($treebranch_array,$lang,'cats');

					$repl_arr['custom_linkidcat:'.$i.':cat_links']='';   

					$repl_sub_arr=array();

					foreach ($treebranch_array as $w){

						$repl_sub_arr['linkdesc']					=	htmlentities($treebranch_data_array[$w]['description'],ENT_COMPAT,'UTF-8');;
						$repl_sub_arr['linktitle']				=	htmlentities($treebranch_data_array[$w]['title'],ENT_COMPAT,'UTF-8');;
						$repl_sub_arr['link']						  =	'<a href="'.$treebranch_data_array[$w]['link'].'">'.
																								(!empty($repl_sub_arr['linktitle'])?$repl_sub_arr['linktitle']:$treebranch_data_array[$w]['link']).
																								'</a>';
						$repl_sub_arr['linkurl']						=	$treebranch_data_array[$w]['link'];
		
						$repl_arr['custom_linkidcat:'.$i.':cat_links'] .= $tpl_links;
	
							foreach ($repl_sub_arr as $k => $v) {
						
								// if-statement
								if(strpos($repl_arr['custom_linkidcat:'.$i.':cat_links'],'{if_'.$k.'}')!==false)
									if (empty($v))
										$repl_arr['custom_linkidcat:'.$i.':cat_links'] = preg_replace('#\{if_'.$k.'\}(.*)\{/if_'.$k.'\}#sU','',$repl_arr['custom_linkidcat:'.$i.':cat_links']);
									else
										$repl_arr['custom_linkidcat:'.$i.':cat_links'] = str_replace(array('{if_'.$k.'}','{/if_'.$k.'}'), array('',''), $repl_arr['custom_linkidcat:'.$i.':cat_links']);
						
								// if-not-statement
								if(strpos($repl_arr['custom_linkidcat:'.$i.':cat_links'],'{if_not_'.$k.'}')!==false)
									if (empty($v))
										$repl_arr['custom_linkidcat:'.$i.':cat_links'] = str_replace(array('{if_not_'.$k.'}','{/if_not_'.$k.'}'), array('',''), $repl_arr['custom_linkidcat:'.$i.':cat_links']);
									else
										$repl_arr['custom_linkidcat:'.$i.':cat_links'] = preg_replace('#\{if_not_'.$k.'\}(.*)\{/if_not_'.$k.'\}#sU','',$repl_arr['custom_linkidcat:'.$i.':cat_links']);
						
								$repl_arr['custom_linkidcat:'.$i.':cat_links']		 =	str_replace('{'.$k.'}',$v,$repl_arr['custom_linkidcat:'.$i.':cat_links']);
						}
				...
				if (!empty($repl_arr['custom_linkidcat:'.$i]) && strpos($tpl_main,':side_links')!==false) {

					$treebranch_array=array();
					$treebranch_data_array=array();
					$treebranch_array=as_get_idcat_children_sides($repl_arr['custom_linkidcat:'.$i],$lang,$client);
					$treebranch_data_array=as_build_treebranch_data_array($treebranch_array,$lang,'sides');
					...
					foreach ($treebranch_array as $w){
						...
						$repl_arr['custom_linkidcat:'.$i.':side_links'] .= $tpl_links;
	
							foreach ($repl_sub_arr as $k => $v) {
						
								// if-statement
								if(strpos($repl_arr['custom_linkidcat:'.$i.':side_links'],'{if_'.$k.'}')!==false)
									if (empty($v))
										$repl_arr['custom_linkidcat:'.$i.':side_links'] = preg_replace('#\{if_'.$k.'\}(.*)\{/if_'.$k.'\}#sU','',$repl_arr['custom_linkidcat:'.$i.':side_links']);
									else
										$repl_arr['custom_linkidcat:'.$i.':side_links'] = str_replace(array('{if_'.$k.'}','{/if_'.$k.'}'), array('',''), $repl_arr['custom_linkidcat:'.$i.':side_links']);
						
								// if-not-statement
								if(strpos($repl_arr['custom_linkidcat:'.$i.':side_links'],'{if_not_'.$k.'}')!==false)
									if (empty($v))
										$repl_arr['custom_linkidcat:'.$i.':side_links'] = str_replace(array('{if_not_'.$k.'}','{/if_not_'.$k.'}'), array('',''), $repl_arr['custom_linkidcat:'.$i.':side_links']);
									else
										$repl_arr['custom_linkidcat:'.$i.':side_links'] = preg_replace('#\{if_not_'.$k.'\}(.*)\{/if_not_'.$k.'\}#sU','',$repl_arr['custom_linkidcat:'.$i.':side_links']);
						
								$repl_arr['custom_linkidcat:'.$i.':side_links']		 =	str_replace('{'.$k.'}',$v,$repl_arr['custom_linkidcat:'.$i.':side_links']);
						}
			...
			$repl_arr['custom_urlencode:'.$i]	=	urlencode($repl_arr['custom:'.$i]);	
			$repl_arr['custom_data_urlencode:'.$i]	=	urlencode($repl_arr['custom_data:'.$i]);	
#			$artsys_obj->settings['article_custom'.$i.'_label']=trim(str_replace('[readonly]','',$artsys_obj->settings['article_custom'.$i.'_label']));
			$artsys_obj->settings['article_custom'.$i.'_label']=strpos($artsys_obj->settings['article_custom'.$i.'_label'],'[')!==false ? trim(substr($artsys_obj->settings['article_custom'.$i.'_label'], 0, strpos($artsys_obj->settings['article_custom'.$i.'_label'],'[')-1)) : $artsys_obj->settings['article_custom'.$i.'_label'];			
	    ...
	    $repl_arr['custom_label:'.$i]	=	htmlentities($artsys_obj->settings['article_custom'.$i.'_label'],ENT_COMPAT,'UTF-8');
			...
			if (trim($artsys_obj->settings['article_custom'.$i.'_alias'])!='') {
				...
				$repl_arr[trim($artsys_obj->settings['article_custom'.$i.'_alias']).'_urlencode']=$repl_arr['custom_urlencode:'.$i];
				...
				$repl_arr[trim($artsys_obj->settings['article_custom'.$i.'_alias']).'_urlencode_data']=$repl_arr['custom_data_urlencode:'.$i];
				...
				$repl_arr[trim($artsys_obj->settings['article_custom'.$i.'_alias']).'_linkidcattree']=$repl_arr['custom_linkidcattree:'.$i];
				$repl_arr[trim($artsys_obj->settings['article_custom'.$i.'_alias']).'_linkidcat:side_links']=$repl_arr['custom_linkidcat:'.$i.':side_links'];
				$repl_arr[trim($artsys_obj->settings['article_custom'.$i.'_alias']).'_linkidcat:cat_links']=$repl_arr['custom_linkidcat:'.$i.':cat_links'];
				...
				$repl_arr[trim($artsys_obj->settings['article_custom'.$i.'_alias']).'_linkidcatsidetree']=$repl_arr['custom_linkidcatsidetree:'.$i];
      ...
      $repl_arr['categoryid']= $item_obj->getDataByKey('idcategory');
		...
		$repl_arr['idcat'] = $idcat;
		$repl_arr['idcatside'] = $idcatside;
		$repl_arr['today_timestamp'] =
	...
	function as_element_ifstatements($tpl='',$elarray,$elkey,$elvalue) {
			...
			if(strpos($tpl,'{if_'.$elkey.'=')!==false) {
				...
				foreach ($_AS['temp']['temp_results'][0] as $ek => $ev) {
					...
					if(strpos($elkey,'linkidcattree')!==false && in_array($_AS['temp']['compval'],explode(',',$elvalue))) {	
						$elvalue=$_AS['temp']['compval'];
					}
