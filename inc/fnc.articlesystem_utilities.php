...
if(! function_exists(as_get_idcatsub_by_cat)){
  ...
  */
  function as_get_idcatsub_by_cat($sf_cat,$sortindex=1,$online=1){
		...
		if ($sf_cattree->getIsGenerated()!==true)
			$sf_cattree->generate();
...
if(! function_exists(as_get_idcatside_by_cat)){
	...
	*/
	function as_get_idcatside_by_cat($sf_cat,$sortindex=1,$online=1){
		...
		$sf_catinfos->setIdlang($sf_lang);
		if ($sf_catinfos->getIsGenerated()!==true)
			$sf_catinfos->generate();
...
if(! function_exists(as_get_idcat_children_cats)){
  /**
  * gets a sub-category of an SF-page-category by sortindex
  *
  * @param		integer $sf_cat -> idcat
  ...
  * @access		public
  */
  function as_get_idcat_children_cats($sf_cat,$sf_lang,$sf_client){
		...
		return $sf_cattree->getChilds($sf_cat);
	
	}
}

if(! function_exists(as_get_idcat_children_sides)){
  /**
  * gets a sub-category of an SF-page-category by sortindex
  *
  * @param		integer $sf_lang -> language id
  * @return		array
  * @access		public
  */
  function as_get_idcat_children_sides($sf_cat,$sf_lang){
		...
		return $sf_pageinfos->getIdcatsidesByIdcat($sf_cat);
	
	}
}

if(! function_exists(as_build_treebranch_data_array)){
  function as_build_treebranch_data_array($sf_catorsidearray,$sf_lang,$mode=''){
		...
		if ($mode=="cats") {
			...
			foreach ($sf_catorsidearray as $v)	{
				$treebranch_data_array[$v]['link']=$sf_catinfos->getLink($v);
				$treebranch_data_array[$v]['title']=$sf_catinfos->getTitle($v);
				$treebranch_data_array[$v]['description']=$sf_catinfos->getDescription($v);
			}
		}
		...
		if ($mode=="sides") {
			...
			foreach ($sf_catorsidearray as $v)	{
				$treebranch_data_array[$v]['link']=$sf_pageinfos->getLink($v);
				$treebranch_data_array[$v]['title']=$sf_pageinfos->getTitle($v);
				$treebranch_data_array[$v]['description']=$sf_pageinfos->getSummary($v);

			}
		}
		
		return $treebranch_data_array;
