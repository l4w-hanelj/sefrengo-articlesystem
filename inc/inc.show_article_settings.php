...
if($_AS['item']->getDataByKey('key2') == 'article_date') {
	...
	for ($i=1;$i<51;$i++){
		if($_AS['item']->getDataByKey('key2') == 'article_custom'.$i.'_label') {
			...
			$_AS['temp']['label']=strip_tags(stripslashes(str_replace(array('<br/>','<br>','<BR/>','<BR>'),
																									 							array('&nbsp;','&nbsp;','&nbsp;','&nbsp;'),
																									 							$_AS['item']->getDataByKey('value',false)
																									 							)));
			$_AS['temp']['label']=strpos($_AS['temp']['label'],'[')!==false ? trim(substr($_AS['temp']['label'], 0, strpos($_AS['temp']['label'],'[')-1)) : $_AS['temp']['label'];			

			if (strlen($_AS['temp']['label'])>32)
				$_AS['temp']['label']=substr($_AS['temp']['label'],0,32).' ...';
			

			$_AS['output']['custom_data'][]=$_AS['temp']['label'];
	...
	for ($i=1;$i<51;$i++){
		if($_AS['item']->getDataByKey('key2') == 'article_custom'.$i.'_alias') {
			...
			$_AS['output']['custom_placeholder'][]='{lng_custom'.$i.'_alias}';

			$_AS['temp']['alias']=stripslashes($_AS['item']->getDataByKey('value',true));
			if (!empty($_AS['temp']['alias']))
				$_AS['output']['custom_data'][]='&nbsp;<span style="color:#666;font-size:0.9em">'.$_AS['temp']['alias'].'&nbsp;<span style="color:#999;">'.$_AS['article_obj']->lang->get('article_custom'.$i).'</span></span>';
			else
				$_AS['output']['custom_data'][]='<span style="color:#999;">'.$_AS['article_obj']->lang->get('article_settings_custom'.$i).'</span>';
